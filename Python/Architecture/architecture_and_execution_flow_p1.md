...
Interview Points:
    ...
    Distinguish the Python compiler from the Python Virtual Machine.


Component	            Responsibility	                Example Error
Tokenizer	            Split source into tokens	    Invalid characters or malformed tokens
Parser	                Validate grammar and build AST	SyntaxError (e.g., missing : or incomplete)
Compiler	            Convert AST to bytecode	        Assumes the AST is already valid
Python Virtual Machine	Execute bytecode	            NameError, TypeError, etc.
